...
export class SkillProjectStore {
  ...
  private seedDefaultProjects() {
    // Wrap existing built-in skills into first-class SkillProjects
    const builtins = [
      {
        ...
        version: '1.0.0',
        projectType: 'single_skill' as const,
    ...
    for (const b of builtins) {
      ...
      const project: SkillProject = {
        ...
        runtimeIntent: {
          ...
          methods: ['规则推演', '结构化提取'],
          skills: [{ id: skill.id, name: skill.displayName || skill.name, role: 'standalone', purpose: skill.description }],
          ...
          dependencies: [],
          outputs: [{ name: '分析成果', format: 'markdown', description: '结构化文档' }],
          verification: [{ check: '关键要素完整性', standard: '覆盖输入所提要求' }],
          constraints: ['遵循客观事实'],
          risks: [],
          completionCriteria: ['任务闭环'],
        },
        compatibility: {
          level: 'L1',
  ...
  /**
   * Install a discovered project into the system
   */
  async installProject(
    discovery: ProjectDiscoveryResult,
    workspaceId?: string
  ): Promise<{ project: SkillProject; diagnosticLogs: InstallDiagnosticStep[] }> {
    const { projectMap, runtimeIntent, compatibilityReport, scannedFiles } = discovery;
    const diagnosticLogs: InstallDiagnosticStep[] = [];

    // Step 1: Verify and log project structure
    diagnosticLogs.push({
    ...
    diagnosticLogs.push({
      ...
      title: '主入口识别完成',
      status: 'success',
      message: `确立主入口为「${projectMap.mainEntry.displayName || projectMap.mainEntry.name}」（角色：${projectMap.mainEntry.role === 'router' ? '总路由入口' : '核心主入口'}）。`,
    });

    // Step 3: Child skills mapping
    diagnosticLogs.push({
      step: 'child_skills',
      title: `下属专项子Skill识别完成 (${projectMap.childSkills.length} 个)`,
      status: 'success',
      message:
        projectMap.childSkills.length > 0
          ? `已成功构建子技能图谱，包含：${projectMap.childSkills.slice(0, 3).map((c) => c.displayName || c.name).join('、')}${projectMap.childSkills.length > 3 ? ` 等共 ${projectMap.childSkills.length} 个专项分支` : ''}。`
          : '本项目为单体聚焦技能，无下属子模块。',
    });
    ...
    diagnosticLogs.push({
      ...
      title: '依赖与工具链检测完成',
      status: 'success',
      message: `所需工具：${projectMap.requiredTools.join('、') || '基础语言模型'}；环境兼容等级：${compatibilityReport.level}。`,
    });
    ...
    diagnosticLogs.push({
      step: 'environment',
      title: '运行环境适配检查完成',
      status: compatibilityReport.status === 'FULL' ? 'success' : 'warning',
    ...
    diagnosticLogs.push({
      ...
      status: 'success',
      message: `已解析作者预期目标、${runtimeIntent.sequence.length} 步执行次序与交付标准，保留原始第三方文件未作任何改动。`,
    });
    ...
    const skillProject: SkillProject = {
      ...
      childSkillsCount: projectMap.childSkills.length,
      toolsCount: projectMap.requiredTools.length,
      resourcesCount:
        projectMap.supportingResources.references.length +
        projectMap.supportingResources.templates.length +
        projectMap.supportingResources.scripts.length,
    };
  ...
  /**
   * Register entry skill and child skills in SkillEngine for Agent Core execution
   */
  private registerSkillsInEngine(
    ...
    const isPack = projectMap.childSkills.length > 0;

    // Helper to find raw content of a skill
    ...
    for (const child of projectMap.childSkills) {
      ...
      const childSkill: SkillMetadata = {
        ...
        role: child.role,
        parentSkillId: projectMap.mainEntry.id,
    ...
    const mainContent = getSkillFile(projectMap.mainEntry.path);
    const mainSkill: SkillMetadata = {
      id: projectMap.mainEntry.id,
      name: projectMap.mainEntry.name,
      displayName: projectMap.mainEntry.displayName,
      description: projectMap.mainEntry.description,
      version: projectMap.version,
      ...
      hasReferences: projectMap.supportingResources.references.length > 0,
      hasTemplates: projectMap.supportingResources.templates.length > 0,
      hasExamples: projectMap.supportingResources.examples.length > 0,
      hasScripts: projectMap.supportingResources.scripts.length > 0,
      files: scannedFiles,
    };
  ...
  toggleProject(id: string, enabled: boolean): boolean {
    ...
    // Also toggle the main entry and child skills
  ...
  deleteProject(id: string): boolean {
    ...
    // Delete skills from SkillEngine
